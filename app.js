...
//   1 country:
//     4 → Country, 5 → Start date, 6 → Return date, 7 → City
//
//   2 countries:
//     8–11   → Stop 1 (country, arrival, departure, city)
//     12–15  → Stop 2
//
//   3 countries:
//     16–19  → Stop 1
...
//     52–55  → Stop 3
//     56–59  → Stop 4
//     60     → Additional countries (free text)
//     61     → Additional date ranges (free text)
//     62     → Additional cities (free text)
//
//   63 → Concerns / questions
//   64 → Select all that apply to your travel
// ═══════════════════════════════════════════════════════════════════
...
async function init() {
  ...
  try {
    ...
    localStorage.setItem("daniher-patients-v1", JSON.stringify(patients.map(p => ({ id: p.id, name: p.name, stops: p.stops || [] }))));
    renderPatients();
